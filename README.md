...
## Part 2 — professional customer-facing features

- Product images: real photography (4 per product) instead of generated
  placeholder art. **Hotlinked from Lorem Picsum, not Unsplash** — this
  sandbox's outbound network policy blocks `images.unsplash.com`, so exact
  Unsplash photo IDs couldn't be verified before shipping, and a broken
  image link is worse than the "wrong" real-photo source. All photo URLs
  are generated by one function (`features/products/lib/productPhotos.ts`),
  so swapping to curated Unsplash URLs (or real S3/CloudFront-hosted
  photography in Phase 2) is a one-file change.
- Product image gallery: thumbnail strip + hover-to-zoom (lens overlay +
  side zoom panel), no library
- Breadcrumbs on product detail, category, and search pages (shared
  `Breadcrumbs` component)
- Recently viewed: `localStorage`-tracked (max 10, no dupes), carousel on
  the homepage and at the bottom of product detail pages
- Ratings breakdown: a clickable star-distribution bar chart above the
  review list, computed from the existing mock review data, filters the
  list in place
- Stock urgency indicators on product cards and the detail page (low-stock
  red/orange text, disabled add-to-cart when out of stock)
- Delivery estimate (business-day date math) and a real delivery fee (free
  ≥ $50, else $4.99) shown on the product page, cart, and as a real line
  item through checkout and order history
- Quick view modal on product grid cards (hover-revealed), without leaving
  the listing page
- Promo codes in checkout (`SAVE10`, `FREESHIP`), applied as a real
  discount line item feeding into the order total
- Order tracking page with a 5-step visual stepper and mock per-step
  estimated dates, linked from the confirmation page and order history
- Order history (`/account/orders`) for signed-in users, with "Buy again"
  (bulk re-add to cart) and "Track order" per order
- Skeleton loading states (with a documented artificial delay) across the
